...
export default function WhyUs() {
  ...
  useEffect(() => {
    const onWheel = (e: WheelEvent) => {
      ...
      if (e.deltaY > 30) {
        if (atEnd) {
          // Last slide — let the page scroll naturally to reveal the footer
          return;
        }
        e.preventDefault();
        goNext();
      } else if (e.deltaY < -30) {
        if (atStart && pageScrolled) {
          // Footer is visible — let the page scroll back up naturally
          return;
        }
        e.preventDefault();
        goPrev();
      } else {
        // Tiny delta — prevent accidental page jumps while between slides
  ...
  return (
    <div className="bg-background text-foreground select-none">
      {/* Slides section — exactly one viewport tall so footer starts below the fold */}
      <main className="h-screen flex flex-col overflow-hidden">
        <div className="max-w-6xl mx-auto w-full px-2 flex flex-col h-full">
          {/* Progress bar */}
